...
import { getSearchVideos } from "../feature/videoThunk";
import {
  formatDuration,
  formatViewCount,
  timeAgo,
...
const SearchPage = () => {
  const { search } = useSelector((state: RootState) => state.global);
  const { hasMore, nextPage, videos } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getSearchVideos({ page: "0", name: search })).finally(() => {
