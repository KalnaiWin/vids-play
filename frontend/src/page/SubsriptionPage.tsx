...
const SubsriptionPage = () => {
  const { subscription } = useSelector((state: RootState) => state.user);
  const { subscriptionVideos, status, hasMore, nextPage } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getSubscriptionVideos({ page: "0" })).finally(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (
          ...
          dispatch(getSubscriptionVideos({ page: String(nextPage) })).finally(
            () => {
