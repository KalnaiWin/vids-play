  ...
  return (
    <div className="text-white ml-5 p-5 mb-10">
      <div className="flex gap-10 items-start w-full">
        ...
        {isVideoTab ? (
          <div
            className="px-4 py-1 rounded-lg bg-white border-3 border-blue-300 text-blue-500 cursor-pointer font-bold"
          ...
          <div
            className="px-4 py-1 rounded-lg bg-white border-3 border-red-300  text-red-500 cursor-pointer font-bold"
            onClick={() => setIsVideoTab(!isVideoTab)}
          >
            󠁯•󠁏 Live
          </div>
      ...
      {isVideoTab ? (
        ...
        <ManagementLiveStream state={isVideoTab} />
      )}
    </div>
  );
};
...
const ManagementVideo = ({ state }: Props) => {
...
const ManagementLiveStream = ({ state }: Props) => {
  const { myRooms, status, statusDelete } = useSelector(
          ...
          <tbody className="divide-y divide-[#1a1a1a]">
            {myRooms.map((room) => (
              ...
              >
                <td className="px-6 py-4">
                  ...
                  >
                    <div className="relative">
                      ...
                      <span className="absolute bottom-1 right-1 bg-black/80 text-[10px] px-1 rounded text-white font-bold">
                        {formatDuration(
                          transferDateToNumberDuration(new Date(room.endedAt)) -
                            transferDateToNumberDuration(
                ...
                <td className="px-6 py-4">
                  <span
                    className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${formatColorStatusStream(room.status)}`}
                  >
                    {room.status}
                  </span>
                </td>
                <td className="px-6 py-4 text-xs text-gray-400 font-semibold">
                  {new Date(room.startedAt).toLocaleDateString("vi-VN")}
                </td>
                <td className="px-6 py-4 text-sm font-medium">
                  {room.totalViews.toLocaleString()}
                </td>
                <td className="px-6 py-4 text-sm font-medium">
                  {room.commentCount}
                </td>
                ...
                <td className="px-6 py-4 text-sm font-medium">
                  <div className="flex items-center justify-around">
                    <Link to={`/edit/room/${room._id}`} title="Chỉnh sửa">
                      <Edit className="text-blue-600 bg-blue-200 rounded-md p-1 size-7 hover:opacity-80 cursor-pointer" />
                    </Link>
                    <button
                      title="Xóa"
                      onClick={() => dispatch(deleteRoom({ id: room._id }))}
                    >
                      <Trash className="text-red-600 bg-red-200 rounded-md p-1 size-7 hover:opacity-80 cursor-pointer" />
                    </button>
                  </div>
